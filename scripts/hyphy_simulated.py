...
if __name__ == '__main__':
    ...
    for file in sorted(os.listdir(args.directory + "/data_prefs")):
        prefix = file.strip().replace(".txt", "")
        hyphy_path = "{0}/data_hyphy/{1}".format(args.directory, prefix)
        if os.path.isdir(hyphy_path) and (protein in prefix):
            nested_dict = nested_dict_init()
            protein_prefs_path = "{0}/data_prefs/{1}".format(args.directory, file)
            batchfiles = [batch[:-3] for batch in os.listdir(hyphy_path) if
                          ((batch[-3:] == ".bf") and (inference in batch))]

            for batch in batchfiles:
                at_gc_pct = float(batch.split("_m")[-1].split("_")[0])
                simu_evol_path = "{0}/data_alignment/{1}_{2}".format(args.directory, prefix,
                                                                     "_".join(batch.split("_")[:-1]))
